# ECS Configuration for Shared Infrastructure
# Container orchestration for AWS
...
# Target Groups (can be replicated for different applications)
resource "aws_lb_target_group" "flask_app" {
  ...
  tags = {
    Name        = "${var.project_name}-${var.environment}-flask-tg"
    Environment = var.environment
    Project     = var.project_name
    Application = "flask"
  }
...
# ALB Listener Rules for different applications
...
resource "aws_lb_listener_rule" "node_app" {
  listener_arn = aws_lb_listener.main.arn
  priority     = 200
